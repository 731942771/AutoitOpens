...
;_ExcelRowInsert
;_ExcelColumnInsert
;_ExcelSheetAddNew			创建新的工作表
;_ExcelSheetDelete
;_ExcelSheetNameGet
;_ExcelSheetNameSet
;_ExcelSheetList			得到全部的工作表名称到一个数组
;_ExcelSheetActivate
;_ExcelSheetMove
...
; #FUNCTION# ====================================================================================================================
; Name...........: _ExcelBookNew
; Description ...: 创建新的工作表并返回对象实例
; Syntax.........: _ExcelBookNew([$fVisible = 1])
; Parameters ....: $fVisible - 脚步执行中是否显示Excel程序 (0=后台执行不显示, 1=显示)
; Return values .: 成功		- 返回新的文件对象标识符
;                  失败		- 返回 0 并设置 @Error:
;                  |@error = 1 - 不能创建 Excel COM 对象
;                  |@error = 2 - $fVisible 参数不是有效数字
;
...
Func _ExcelBookNew($fVisible = 1)
	Local $oExcel = ObjCreate("Excel.Application") ; 微软
	If Not IsObj($oExcel) Then
		$oExcel = ObjCreate("et.Application") ; 金山wps
...
; #FUNCTION# ====================================================================================================================
; Name...........: _ExcelBookOpen
; Description ...: 打开一个现有的（已存在的）工作簿并返回其对象标识符
; Syntax.........: _ExcelBookOpen($sFilePath[, $fVisible = 1[, $fReadOnly = False[, $sPassword = ""[, $sWritePassword = ""]]]])
; Parameters ....: $sFilePath - 要操作的文件路径
;                  $fVisible - 隐藏或显示文件操作 (0=not visible, 1=visible) (default=1)
;                  $fReadOnly - 是否只读方式打开文件(True or False) (default=False)
;                  $sPassword - 如果需要，阅读密码 (default is none)
;                  $sWritePassword - 如果需要，修改密码 (default is none)
...
Func _ExcelBookOpen($sFilePath, $fVisible = 1, $fReadOnly = False, $sPassword = "", $sWritePassword = "")
	...
	If Not IsObj($oExcel) Then
		$oExcel = ObjCreate("et.Application")
		If Not IsObj($oExcel) Then Return SetError(1, 0, 0)
	EndIf
...
;                  附加依据所选模式搜索字符串的匹配方法到一个工作簿的第一个实例
...
Func _ExcelBookAttach($s_string, $s_mode = "FilePath")
	...
	If @error Or Not IsObj($o_Result) Then
		$o_Result = ObjGet("", "et.Application")
		If @error Or Not IsObj($o_Result) Then Return SetError(1, 1, 0)
...
; Syntax.........: _ExcelBookSave($oExcel[, $fAlerts = 0]) 保存指定的工作簿.
; Parameters ....: $oExcel - Excel object opened by a preceding call to _ExcelBookOpen() or _ExcelBookNew()
;                            调用 _ExcelBookOpen() or _ExcelBookNew() 打开的Excel对象
...
;                  写入一行(文件，行，列，数组，方向 0-水平-行/1-垂直-列，)
...
;                  将二维数组写入活动工作表（文件，数组，开始行，开始列，数组开始行索引，数组开始列索引）
...
; Parameters ....: $oExcel - 通过_ExcelBookOpen() or _ExcelBookNew()打开的一个 Excel object
;                  $sName - 工作表名称(default follows standard Excel new sheet convention)
; Return values .: Success      - Returns 1
;                  Failure		- Returns 0 and sets @error on errors:
;                  |@error=1     - Specified object does not exist
; Author ........: SEO <locodarwin at yahoo dot com>
...
; Example .......: Yes
; ===============================================================================================================================
Func _ExcelHorizontalAlignSet($oExcel, $sRangeOrRowStart, $iColStart = 1, $iRowEnd = 1, $iColEnd = 1, $sHorizAlign = "left", $autoFit = True)
	If Not IsObj($oExcel) Then Return SetError(1, 0, 0)
	If Not StringRegExp($sRangeOrRowStart, "[A-Z,a-z]", 0) Then
		If $sRangeOrRowStart < 1 Then Return SetError(2, 0, 0)
		If $iColStart < 1 Then Return SetError(2, 1, 0)
		...
		If $autoFit = True Then
			$oExcel.Activesheet.Range($oExcel.Cells($sRangeOrRowStart, $iColStart), $oExcel.Cells($iRowEnd, $iColEnd)).EntireColumn.AutoFit()
		EndIf
	Else
		Switch ($sHorizAlign)
			Case "left"
		...
		EndSwitch
		If $autoFit = True Then
			$oExcel.Activesheet.Range($sRangeOrRowStart).EntireColumn.AutoFit()
		EndIf
	EndIf
	Return 1
...
;                  设置属性。对已有的行列生效（文件，起始行，起始列，结束行，结束列，字体，粗体，斜体，字号，下划线，颜色，行高，列宽, 对齐，底色）
...
; Example .......: Yes
; ===============================================================================================================================
;    _ExcelFontSetProperties($oExcel, 开始行,            开始列,         结束行,       结束列,       "字体",       粗体,           斜体,             字号,       下划线,              文本颜色,         行高,            列宽,              水平对齐,             自动列宽)
Func _ExcelFontSetProperties($oExcel, $sRangeOrRowStart, $iColStart = 1, $iRowEnd = 1, $iColEnd = 1, $font="宋体", $fBold = False, $fItalic = False, $size = 12, $fUnderline = False, $colorindex = 1, $rowHeight = 12, $columnwidth = 13, $sHorizAlign = "left", $autoFit = False)
	If Not IsObj($oExcel) Then Return SetError(1, 0, 0)
	If Not StringRegExp($sRangeOrRowStart, "[A-Z,a-z]", 0) Then
		If $sRangeOrRowStart < 1 Then Return SetError(2, 0, 0)
		If $iColStart < 1 Then Return SetError(2, 1, 0)
		If $iRowEnd < $sRangeOrRowStart Then Return SetError(3, 0, 0)
		If $iColEnd < $iColStart Then Return SetError(3, 1, 0)
		$oExcel.Activesheet.Range($oExcel.Cells($sRangeOrRowStart, $iColStart), $oExcel.Cells($iRowEnd, $iColEnd)).Font.Name = $font
		...
		$oExcel.Activesheet.Range($oExcel.Cells($sRangeOrRowStart, $iColStart), $oExcel.Cells($iRowEnd, $iColEnd)).Font.Size = $size
		$oExcel.Activesheet.Range($oExcel.Cells($sRangeOrRowStart, $iColStart), $oExcel.Cells($iRowEnd, $iColEnd)).Font.Underline = $fUnderline
		$oExcel.Activesheet.Range($oExcel.Cells($sRangeOrRowStart, $iColStart), $oExcel.Cells($iRowEnd, $iColEnd)).Font.Colorindex = $colorindex
		$oExcel.Activesheet.Range($oExcel.Cells($sRangeOrRowStart, $iColStart), $oExcel.Cells($iRowEnd, $iColEnd)).RowHeight = $rowHeight
		$oExcel.Activesheet.Range($oExcel.Cells($sRangeOrRowStart, $iColStart), $oExcel.Cells($iRowEnd, $iColEnd)).Columnwidth = $columnwidth
		Switch ($sHorizAlign)
			Case "left"
				$oExcel.Activesheet.Range($oExcel.Cells($sRangeOrRowStart, $iColStart), $oExcel.Cells($iRowEnd, $iColEnd)).HorizontalAlignment = $xlLeft
			Case "center", "centre"
		...
		EndIf
	Else
		$oExcel.Activesheet.Range($sRangeOrRowStart).Font.Name = $font
		$oExcel.Activesheet.Range($sRangeOrRowStart).Font.Bold = $fBold
		$oExcel.Activesheet.Range($sRangeOrRowStart).Font.Italic = $fItalic
		$oExcel.Activesheet.Range($sRangeOrRowStart).Font.Size = $size
		$oExcel.Activesheet.Range($sRangeOrRowStart).Font.Underline = $fUnderline
		$oExcel.Activesheet.Range($sRangeOrRowStart).Font.Colorindex = $colorindex
		$oExcel.Activesheet.Range($sRangeOrRowStart).RowHeight = $rowHeight
		$oExcel.Activesheet.Range($sRangeOrRowStart).Columnwidth = $columnwidth
		Switch ($sHorizAlign)
			Case "left"
				$oExcel.Activesheet.Range($sRangeOrRowStart).HorizontalAlignment = $xlLeft
			Case "center", "centre"
